...
//							}
			...
			case Gestures.KickRight:
				...
				break;
            //check for ButtomRight
            case Gestures.ButtomRight:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] 
                            //&& jointsPos[shoulderCenterIndex].x - jointsPos[hipCenterIndex].x > -0.1f 
                            )
                        {
                            SetGestureJoint(ref gestureData, timestamp, hipCenterIndex, jointsPos[hipCenterIndex]);
                        }
                    ...
                    case 1:
                        if (timestamp - gestureData.timestamp < 1.0f)
                        {
                            bool isInPose = jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsPos[shoulderCenterIndex].x - jointsPos[hipCenterIndex].x < -0.02f;
            ...
            //check for DoubleHandRight
            case Gestures.DoubleHandRight:
                switch(gestureData.state)
                {
                    case 0:
                        if (jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && 
                           jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= jointsPos[shoulderCenterIndex].y //&&
                           //    jointsPos[leftHandIndex].x <= gestureLeft && jointsPos[leftHandIndex].x > gestureRight
                           )
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
                    ...
                    case 1:
                        if (timestamp - gestureData.timestamp < 1.5f)
                        {
                            bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] 
                                && jointsPos[leftHandIndex].x < gestureRight && jointsPos[rightHandIndex].x < gestureRight 
                               //&& jointsPos[leftHandIndex].y - jointsPos[shoulderCenterIndex].y <0.2f
                               ;
            ...
            //check for DoubleHandRight
            case Gestures.DoubleHandLeft:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] &&
                           jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= jointsPos[shoulderCenterIndex].y //&&
                           //    jointsPos[leftHandIndex].x <= gestureLeft && jointsPos[leftHandIndex].x > gestureRight
                           )
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                    ...
                    case 1:
                        if (timestamp - gestureData.timestamp < 1.5f)
                        {
                            bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex]
                                && jointsPos[rightHandIndex].x > gestureLeft && jointsPos[leftHandIndex].x > gestureLeft
                               //&& jointsPos[leftHandIndex].y - jointsPos[shoulderCenterIndex].y <0.2f
                               ;
            ...
            //check for LeftHandDownRight
            case Gestures.LeftHandDownRight:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[leftHandIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[rightHandIndex]
                           // && jointsPos[leftHandIndex].y - jointsPos[shoulderCenterIndex].y < 0.2f
                            //&& jointsPos[leftHandIndex].y - jointsPos[shoulderCenterIndex].y > -0.2f 
                           // && jointsPos[rightHandIndex].y - jointsPos[shoulderCenterIndex].y < 0.2f
                           // && jointsPos[rightHandIndex].y - jointsPos[shoulderCenterIndex].y > -0.2f
                           )
                        {
                    ...
                    case 1:
                        if (timestamp - gestureData.timestamp < 1.5f)
                        {
                            bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex]
                             && jointsPos[leftHandIndex].x > jointsPos[leftShoulderIndex].x
                             && jointsPos[leftHandIndex].x < jointsPos[rightShoulderIndex].x;
            ...
            //check for RightHandDownRight

            //check for DoubleHandPullBack
            case Gestures.DoubleHandPullBack:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                    ...
                    case 1:  // gesture phase 2 = complete
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex]
                                && gestureData.jointPos.y > jointsPos[rightHandIndex].y;
